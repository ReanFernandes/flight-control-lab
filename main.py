...
def main():
    ...
    R = np.diag([1, 1, 1, 1])* 0.006

    # define the starting point
    x_init = np.array([1, 0, 1])
    x_desired = np.array([3, 3, 7])

    # options for ipopt
    nlpopts = {'ipopt': {'print_level': 0}, 'print_time' : 0}
    ...
    numerical_method = "direct_collocation"
    degree = 2
    X_mpc_dc, U_mpc_dc, deviation_dc, _ = MPC(degree ,100*Q, R, function_type, x_init, x_desired, N, T, Tf, numerical_method, nlpopts)
